...
/*
    ...
    then the script is reloaded.  instead, wrap underscore in a define() call
    and let it export _ via the exports parameter instead of a global.

    because of a bug in the FW JS engine, the regular expressions that 
    require.js uses to check if a callback function contains calls to require
    throw an error, so we explicitly specify the dependencies, which normally
    wouldn't be necessary.

    the underscore methods that make use of setTimeout() have been removed from
    this file, as the FW JS engine doesn't support that function.
*/
