...
/*
  ...
  Done:
  	- create namespace() function in fwlib, or loadModule()
  		loadModule("com.johndunning.layers") would create fwlib.com.johndunning.layers = {}
...
// ===========================================================================
// 	fwlib.layers package
// ===========================================================================

define(function() {

	// for now, just set up a local version of the fwlib global so that we
	// don't have to change the code below.  we'll return just fwlib.layers 
	// as the API.
var fwlib = { layers: {} };
...
//	LayerTree
// ===========================================================================
fwlib.layers.LayerTree = function(
	inAttributes)
{
	...
	this.root = new fwlib.layers.Layer({
		index: -1,
		parent: null,
		dom: this.dom,
		tree: this
	...
	for (var i = 0, len = this.dom.layers.length; i < len; i++) {
		var layer = this.layers[i] = new fwlib.layers.Layer({
			index: i,
...
// ===========================================================================
fwlib.layers.LayerTree.prototype.dom = null;
fwlib.layers.LayerTree.prototype.layers = null;
fwlib.layers.LayerTree.prototype.root = null;


// ===========================================================================
fwlib.layers.LayerTree.prototype.getLayerByIndex = function(
	inIndex)
{
...
// ===========================================================================
fwlib.layers.LayerTree.prototype.getCurrentLayer = function()
{
...
// ===========================================================================
fwlib.layers.LayerTree.prototype.getTopLayers = function()
{
...
// ===========================================================================
fwlib.layers.Layer.prototype.index = -1;
fwlib.layers.Layer.prototype.sublayerIndex = -1;
fwlib.layers.Layer.prototype.name = "";
fwlib.layers.Layer.prototype.dom = null;
fwlib.layers.Layer.prototype.tree = null;


// ===========================================================================
fwlib.layers.Layer.prototype.getLayer = function() 
{
...
// ===========================================================================
fwlib.layers.Layer.prototype.getElements = function(
	inFrame)
{
...
// ===========================================================================
fwlib.layers.Layer.prototype.isVisible = function(
	inFrame)
{
	if (typeof inFrame != "number") {
		inFrame = this.dom.currentFrameNum;
...
// ===========================================================================
fwlib.layers.Layer.prototype.isLocked = function(
	inFrame)
{
	if (typeof inFrame != "number") {
		inFrame = this.dom.currentFrameNum;
...
// ===========================================================================
fwlib.layers.Layer.prototype.isOpen = function()
{
...
// ===========================================================================
fwlib.layers.Layer.prototype.isTopLayer = function()
{
...
// ===========================================================================
fwlib.layers.Layer.prototype.getTopLayerAncestor = function()
{
	var layer = this;
...
// ===========================================================================
fwlib.layers.Layer.prototype.getTopLayerAbove = function()
{
...
// ===========================================================================
fwlib.layers.Layer.prototype.getTopLayerBelow = function()
{
	var topLayer = this.getTopLayerAncestor();
	var topLayers = this.tree.getTopLayers();
...
// ===========================================================================
fwlib.layers.Layer.prototype.getLayerAbove = function()
{
...
// ===========================================================================
fwlib.layers.Layer.prototype.getLayerBelow = function()
{
...
// ===========================================================================
fwlib.layers.Layer.prototype.toString = function()
{
...
//	copyLayer
// ===========================================================================
fwlib.layers.copyLayer = function(
	inLayer,
	inTargetDom,
	inSourceDom)
{
	...
	var targetLayerIndex = fwlib.layers.getLayerIndexByName(sourceLayer.name, inTargetDom);
	...
	if (targetLayerIndex == -1) {
		...
		if (!inLayer.isTopLayer()) {
			...
			var targetParentIndex = fwlib.layers.getLayerIndexByName(inLayer.parent.name, inTargetDom);
			...
			inTargetDom.addNewSubLayer(targetParentIndex, sourceLayer.name, sourceLayer.sharing);
				...
				// the web layer.  
			inTargetDom.currentLayerNum = fwlib.layers.getTopAncestorLayerIndex(inTargetDom.layers.length - 1, inTargetDom);
...
// ===========================================================================
//	copyLayerBetweenPages
// ===========================================================================
fwlib.layers.copyLayerBetweenPages = function(
	inLayer,
	inSourcePageIndex,
	inTargetPageIndex)
{
	...
	if (!inLayer.isTopLayer()) {
			// we need to create a sublayer, so find the index of the parent
			// layer, which may be different than in the source doc
		var targetParentIndex = fwlib.layers.getLayerIndexByName(inLayer.parent.name, targetDom);
...
// check for parent layer not found
		...
		targetDom.addNewSubLayer(targetParentIndex, sourceLayer.name, sourceLayer.sharing);
			...
			// layer because layers[layers.length - 1] might be a sublayer of
			// the web layer.  
		targetDom.currentLayerNum = fwlib.layers.getTopAncestorLayerIndex(targetDom.layers.length - 1, targetDom);
...
//	getLayerIndexByName
// ===========================================================================
fwlib.layers.getLayerIndexByName = function(
	inLayerName,
	inDom)
{
...
// ===========================================================================
//	getTopAncestorLayerIndex
// ===========================================================================
fwlib.layers.getTopAncestorLayerIndex = function(
	inLayerIndex,
	inDom) 
{
...
//	getNextDocument
// ===========================================================================
fwlib.layers.getNextDocument = function(
	inDom)
{
	var dom = inDom || fw.getDocumentDOM();
...
//	getTopLayerIndexes
// ===========================================================================
fwlib.layers.getTopLayerIndexes = function(
	inDom) 
{
	var dom = inDom || fw.getDocumentDOM();
	var layerIndexes = [];
...
//	alertLayers
// ===========================================================================
fwlib.layers.alertLayers = function(
	inDom,
	inJustReturnOutput)
{
	inDom = inDom || fw.getDocumentDOM();

	var tree = new fwlib.layers.LayerTree({ dom: inDom });
